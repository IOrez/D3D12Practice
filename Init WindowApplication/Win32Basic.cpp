...
/*
	...
	함수. 초기화에 성공하면 true를, 그렇지 않으면 false를 돌려준다.	
*/
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPreInstance, PSTR pCmdLine, int nShowCmd) {
	/* 
		우선 hInstance와 nShowCmd로 초기화 함수(InitWindowsApp)를 호출해서
		응용 프로그램 주 창을 초기화 한다.
	*/
...
bool InitWindowsApp(HINSTANCE instanceHandle, int show){
	...
	*/
	...
	/*
		...
		Windows가 내부적으로 관리한다. 창을 다루는 Win32 API 함수 중에는 자신이 작업할 창을 식별
		하기 위해 이 HWND 값을 받는 것들이 많다.
	*/
	...
	ghMainWnd = CreateWindow(
		...
		CW_USEDEFAULT,				// 창의 위치의 x좌표의 성분
		CW_USEDEFAULT,				// 창의 위치의 y좌표의 성분
		CW_USEDEFAULT,				// 창의 너비
		CW_USEDEFAULT,				// 창의 높이
		0,							// 부모 창 핸들
		0,							// 메뉴 핸들
	...
	/*
		...
		실제 화면에 표시하고 갱신하기 위해서는 다음 두 함수를 호출해
		주어야 한다. 두 함수 모두, 표시 또는 갱신할 창의 핸들을 받는다는
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam){
	...
	*/
	...
	case WM_LBUTTONDOWN:
		...
		//ESC 키가 눌렸으면 주 응용 프로그램의 창을 파괴한다.
	case WM_KEYDOWN:
		if (wParam == VK_ESCAPE) {
	...
	/*
		...
		반드시 DefWindowProc의 반환값을 돌려 주어야 함을 주의하기
		바란다.
	*/
